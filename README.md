...
3. `{管理フォルダ名}/admin.php` にアクセスして管理者パスワードを設定

サイト固有のデータは、初回アクセス時に `index.php` と同じ階層の `mikanData` へ作成されます。旧 `mikanBox/data` があるサイトは、安全に移動できる場合だけ自動移行されます。
...
ワンクリックで全ページを静的HTMLとして書き出せます。静的・動的ページを混在させることも可能です。サーバー内に生成する方式と、相対リンク化したHTMLとmediaフォルダをまとめるアップロード用出力を選択できます。
...
Model Context Protocol (MCP) に対応しており、AIエージェントがサーバー上のファイルを安全に操作するための接続機能を提供します。これにより、ユーザーが管理画面を操作することなく、AIとの対話だけでページの新設やデザインの修正、コンポーネントの作成が完結します。

管理用MCPは、MCP 2026-07-28と、`initialize` を使うMCP 2025-11-25／2025-06-18／2025-03-26に対応します。接続には `Authorization: Bearer` または `X-API-Key` ヘッダーで管理用APIキーを送信してください。ClaudeカスタムコネクタのOAuth Client ID／シークレット欄はAPIキー欄ではないため、管理用MCPはmikanBoxがOAuthへ対応するまで同コネクタへ登録できません。

複数のmikanBoxサイトを同時に接続する場合は、`get_site_info` ツールがサイト固有のID、サイト名、公開URL、環境（本番・ステージング・開発・ローカル）を返します。AIは書き込み操作の前に接続先を確認できるため、サイトの取り違えを防ぎやすくなります。

2.5.1以降は、接続直後に `get_ai_context` を使い、DESIGN.md・BRAND.md・CONTENTS.mdなどのAI指示コンポーネントを一度に読み込みます。分けて管理した指示を、そのサイト固有のプロジェクト指示として効率よくAIへ伝えられます。
