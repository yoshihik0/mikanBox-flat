...
try {
    ...
    $url = "http://127.0.0.1:{$port}/mikanBox/mcp.php";
    $baseHeaders = [
    ...
    checkHttp($legacy['status'] === 400, 'legacy initialize must return HTTP 400');
    checkHttp(($legacy['json']['error']['code'] ?? null) === -32022, 'legacy initialize must return UnsupportedProtocolVersion');
